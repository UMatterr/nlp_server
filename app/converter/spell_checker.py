...
from converter.response import Checked
from converter.constants import base_url
from converter.constants import CheckResult
from converter.constants import CALLBACK, PASSPORT_KEY
...
def check(text):
    ...
    json_txt = r.text.replace(CALLBACK,'')
    if json_txt.startswith('(') == True:
        json_txt = json_txt[1:]
    if json_txt.endswith(');') == True:
        json_txt = json_txt[:-2]
