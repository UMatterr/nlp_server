...
def generate_chuseok():
	# 이부분이 실제 train 된 모델로 호출하는 부분
	return generate('추석8565', tokenizer, model, 1)
...
def get_dummy_convert(src, convert_type):
    ...
    if convert_type == 'respectful':
        if str(src).endswith('요') == False:
            return src + '요'
        else :
            return src
    elif convert_type == 'informal':
        if str(src).endswith('요') == True:
            return src[:-1]
        else :
            return src
    else:
...
class Converted(Resource):
    ...
    def get(self, how):
        ...
        Examples 1: GET /converted/respectful?src=생일축하해                              <- 존댓말 변환
        ...
        if (how == 'respectful') & none_or_empty(src):
            src = '생일축하해'
...
if __name__ == "__main__":
	...
	model = GPT2LMHeadModel.from_pretrained("UMatterr/chuseok").to(device)
